...
def register(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(
                request, "note/register.html", {"message": "Passwords must match."}
            )
        ...
        try:
            ...
            return render(
                request, "note/register.html", {"message": "Username already taken."}
            )
        login(request, user)
        return HttpResponseRedirect(reverse("index"))
    else:
...
def allnote(request):
    ...
    return render(
        request,
        "note/allnote.html",
        {"notes": unpinned_notes, "pinned_notes": pinned_notes},
    )
